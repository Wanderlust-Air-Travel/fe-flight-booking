...
import type { FareClass, Route, RouteFarePrice } from "@/types/admin/route-fare-price-type";
...
export default function RouteFarePricesPage() {
        ...
        const transformedPrices = (responseData.data || []).map((price: any) => {
          ...
          if (price.route) {
            ...
            const destinationAirport =
              price.route.destination_airport || price.route.destinationAirport;
          ...
          return {
            ...
            basePrice:
              typeof price.base_price === "number" ? price.base_price : price.basePrice || 0,
            ...
            effectiveFrom: price.effective_from
              ? typeof price.effective_from === "string"
                ? price.effective_from
                : new Date(price.effective_from).toISOString()
              : "",
            effectiveTo: price.effective_to
              ? typeof price.effective_to === "string"
                ? price.effective_to
                : new Date(price.effective_to).toISOString()
              : null,
  ...
  const handleCreate = async () => {
    try {
      ...
      await axiosInstance.post("/api/admin/route-fare-prices", {
        ...
        feeRate: Number.parseFloat(formData.feeRate),
  ...
  const handleEdit = (price: RouteFarePrice) => {
    ...
    const effectiveFromDate =
      typeof price.effectiveFrom === "string" ? price.effectiveFrom.split("T")[0] : "";
    const effectiveToDate =
      price.effectiveTo && typeof price.effectiveTo === "string"
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#00558f] to-[#3775A4] p-8 text-white shadow-lg">
        <div className="relative z-10">
          ...
          <p className="text-blue-50 text-lg">
            Quản lý và điều chỉnh giá vé cho từng route và hạng vé
          </p>
        </div>
        {/* Decorative elements */}
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="grid gap-3">
                      ...
                      <Input
                        ...
                        type="number"
                        step="0.01"
                        value={(Number.parseFloat(formData.taxRate) * 100).toString()}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            taxRate: (Number.parseFloat(e.target.value) / 100).toString(),
                          })
                        }
                        placeholder="10"
                        min="0"
                    ...
                    <div className="grid gap-3">
                      ...
                      <Input
                        ...
                        type="number"
                        step="0.01"
                        value={(Number.parseFloat(formData.feeRate) * 100).toString()}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            feeRate: (Number.parseFloat(e.target.value) / 100).toString(),
                          })
                        }
                        placeholder="5"
                        min="0"
            ...
            <Table>
              ...
              <TableBody>
                {routeFarePrices.length === 0 ? (
                  ...
                  routeFarePrices.map((price, _index) => (
                    <TableRow
