...
import { calculateAge, determinePassengerType } from "@/lib/passenger-utils";
      ...
      <main className="flex flex-col pt-[var(--hd)] gap-y-[var(--rowY)]">
        ...
        <div className="container">
          <div className="text-center py-8 md:py-12 lg:py-16">
            ...
            <Button onClick={() => router.back()} className="mt-4 md:mt-6 lg:mt-8">
              {tCommon("back")}
            </Button>
          </div>
        </div>
            ...
            <div className="w-full lg:w-[70%]">
              <div className="bg-white rounded-lg md:rounded-xl lg:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 xl:p-10 border border-[var(--cl-third)] shadow-md">
                  ...
                  <Alert className="mb-4 md:mb-6 lg:mb-8 border border-[var(--cl-third)] bg-blue-50">
                    <AlertDescription className="text-sm md:text-base">
                      ...
                      <p className="text-gray-700 text-sm md:text-base">
                        Expires at:
                        <strong>{new Date(reservationData.expiresAt).toLocaleString()}</strong>
                      </p>
                    </AlertDescription>
                  </Alert>
                )}
                ...
                {isCountdownSubscribed && countdown && (
                  <Alert
                    className={`mb-4 md:mb-6 lg:mb-8 border-2 ${isExpired ? "border-red-500 bg-red-50" : remainingSeconds < 300 ? "border-yellow-500 bg-yellow-50" : "border-[var(--cl-pri)] bg-blue-50"}`}
                  >
                    <AlertDescription>
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                        <div className="flex-1">
                          <p
                            className={`font-bold text-base md:text-lg lg:text-xl ${isExpired ? "text-red-600" : remainingSeconds < 300 ? "text-yellow-700" : "text-[var(--cl-pri)]"}`}
                          >
                            {isExpired ? t("reservationExpired") : t("reservationRemaining")}
                          </p>
                        ...
                        <div className="text-left sm:text-right">
                          <p
                            className={`text-2xl md:text-3xl lg:text-4xl font-bold ${isExpired ? "text-red-600" : remainingSeconds < 300 ? "text-yellow-700" : "text-[var(--cl-pri)]"}`}
                          >
                            {formattedCountdown}
                          </p>
                          {isExpired && (
                    ...
                    <Form className="flex flex-col gap-y-6 md:gap-y-8 lg:gap-y-12">
                      ...
                      <div className="flex flex-col gap-y-4 md:gap-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                          <FormField name="contactFullname" label={t("fullName")} required>
                            {({ field, className }) => (
                              <Input
                                {...field}
                                type="text"
                          ...
                          <FormField name="contactEmail" label={t("contactEmail")} required>
                            {({ field, className }) => (
                              <Input
                                {...field}
                                type="email"
                          ...
                          <div className="md:col-span-2">
                            <FormField name="contactPhone" label={t("contactPhone")} required>
                              {({ field, className }) => (
                                <Input
                                  {...field}
                                  type="tel"
                      ...
                      {user && (
                        <div className="flex flex-col gap-y-4 md:gap-y-6 p-4 md:p-6 lg:p-8 bg-gradient-to-br from-blue-50 to-white border border-[var(--cl-third)] rounded-lg md:rounded-xl shadow-sm">
                          <div className="flex items-center gap-2 md:gap-3">
                            ...
                            />
                            <Label
                              htmlFor="isUserTraveling"
                              className="cursor-pointer text-sm md:text-base lg:text-lg font-bold text-[var(--cl-pri)]"
                            >
                              ...
                              <div className="flex flex-wrap gap-2 md:gap-3 mt-2 md:mt-3">
                                {values.passengers.map((_, idx) => (
                                  <Button
                                    ...
                                    type="button"
                                    variant={
                                      values.userPassengerIndex === idx ? "default" : "outline"
                                    }
                                    size="sm"
                                    className={
                                      values.userPassengerIndex === idx
                                    ...
                                    onClick={() => {
                                      ...
                                      if (prevIndex !== undefined) {
                                        setFieldValue(
                                          `passengers.${prevIndex}.isCurrentUser`,
                                          false
                                        );
                      ...
                      <div className="flex flex-col gap-y-4 md:gap-y-6 lg:gap-y-8">
                        ...
                        {values.passengers.map((passenger, index) => {
                          ...
                          const _isChild = passenger.passengerType === "CHD";
                          const adultCount = values.passengers.filter(
                            (p) => p.passengerType === "ADT"
                          ).length;
                          const infantCount = values.passengers.filter(
                              ...
                              {isInfant && (
                                <Alert className="mb-4 md:mb-6 border border-[var(--cl-third)] bg-blue-50">
                                  <AlertDescription className="text-sm md:text-base lg:text-lg">
                                    <p className="font-bold text-[var(--cl-pri)] mb-2 md:mb-3 text-sm md:text-base lg:text-lg">
                                      Infant Requirements:
                                    </p>
                                    ...
                                    {infantCount > adultCount && (
                                      <p className="mt-2 md:mt-3 font-bold text-destructive text-sm md:text-base lg:text-lg">
                                        Warning: You have {infantCount} infant(s) but only
                                        {adultCount} adult(s). Each adult can only accompany 1
                                        infant. Additional infant(s) must be booked as Child (CHD).
                                      </p>
                                    )}
                                  </AlertDescription>
                                </Alert>
                                  ...
                                  (() => {
                                    ...
                                    if (age >= 0 && determinedType) {
                                      ...
                                      return (
                                        <div className="md:col-span-2">
                                          <p
                                            className={`text-sm md:text-base mt-2 font-medium ${typeMatches ? "text-[var(--cl-four)]" : "text-orange-600"}`}
                                          >
                                            {typeMatches ? (
                                              <span>
                                                ✓ Tuổi tại ngày bay: {age} tuổi - Loại hành khách
                                                phù hợp: {determinedType}
                                              </span>
                                            ) : (
                                              <span>
                                                ⚠ Tuổi tại ngày bay: {age} tuổi - Đề xuất loại:
                                                {determinedType} (hiện tại:
                                      ...
                                      >
                                        <div className="flex items-center space-x-2 md:space-x-3">
                                          ...
                                          <Label
                                            ...
                                            className="text-sm md:text-base lg:text-lg font-semibold text-[var(--cl-pri)] cursor-pointer"
                                          >
                                            Male
                                          </Label>
                                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 md:gap-4 lg:gap-6 pt-4 md:pt-6 lg:pt-8 border-t-2 border-[var(--cl-pri)]">
                        ...
                        >
                          {isCreatingBooking ? t("creatingBooking") : t("continuePayment")}
                        </Button>
                      </div>
                    </Form>
                  )}
                ...
                {reservationData && (
                  <div className="flex flex-col gap-y-4 md:gap-y-6">
                    ...
                    <div className="flex justify-between items-center p-3 md:p-4 lg:p-6 bg-white rounded-lg md:rounded-xl border border-[var(--cl-third)]">
                      ...
                      <span className="text-sm md:text-base lg:text-lg font-bold text-[var(--cl-pri)]">
                        {reservationData.currencyCode || "VND"}
                      </span>
                    </div>
                  </div>
                )}
