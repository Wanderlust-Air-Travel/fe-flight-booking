...
const SeatMapPageContent = () => {
  ...
  const [selectedSeatsInfo, setSelectedSeatsInfo] = useState<
    Array<{ flightSeatId: string; seatNumber: string }>
  >([]);
  ...
  const { isSubscribed, seatChanges, applySeatChanges, clearSeatChanges } =
    useSeatAvailability(flightInstanceId);

  // Redirect nếu không có flightInstanceId
  useEffect(() => {
    if (!flightInstanceId) {
      router.push(localizedHref("/search/flights", locale));
      return;
    }
  ...
  const findSeatBySeatId = useCallback(
    (seatId: string): SeatItem | null => {
      const allSeats = [...(seatBusiness?.list || []), ...(seatEconomy?.list || [])];
      // Try to find by flightSeatId first (primary method)
      let foundSeat: SeatItem | null =
        allSeats.find((seat) => seat.flightSeatId === seatId) || null;

      // If not found by flightSeatId, try by generated ID pattern (fallback for legacy support)
      if (!foundSeat) {
        const fallbackSeat = allSeats.find(
          (seat) =>
    ...
    (cabinType: "business" | "economy") => (seatId: string, checked: boolean) => {
      ...
      if (checked) {
        ...
        if (selectedSeatsInfo.length >= passengersNeedingSeats) {
          setSaveError(
            `You can only select ${passengersNeedingSeats} seat(s) for ${passengersNeedingSeats} passenger(s) (${searchBarData?.adult || 0} adult(s) + ${searchBarData?.child || 0} child(ren)). Infants do not require seats.`
          );
          return;
        }
        ...
        setSelectedSeatsInfo((prev) => prev.filter((s) => s.flightSeatId !== seat.flightSeatId));
  ...
  const handleContinue = useCallback(async () => {
    ...
    if (selectedSeatsInfo.length === 0) {
      setSaveError(tCommon("required"));
      return;
    }

    if (selectedSeatsInfo.length < seatsNeeded) {
      setSaveError(
        `Please select ${seatsNeeded} seat(s) for ${seatsNeeded} passenger(s) (${searchBarData?.adult || 0} adult(s) + ${searchBarData?.child || 0} child(ren)). Infants do not require seats.`
      );
      return;
    }
    ...
    if (selectedSeatsInfo.length > seatsNeeded) {
      setSaveError(
        `You have selected ${selectedSeatsInfo.length} seat(s), but only ${seatsNeeded} seat(s) are needed (${searchBarData?.adult || 0} adult(s) + ${searchBarData?.child || 0} child(ren)). Infants do not require seats.`
      );
      return;
    }
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setSaveError(response.data?.message || tBk("paymentFailed"));
        return;
      }
          ...
          <div className="flex flex-wrap -mx-[1.2rem]">
            <div className="px-[1.2rem] w-[70%]">
              <div className="pt-[calc(100%*6000/2000)] w-full relative overflow-hidden block">
                ...
                <div
                  className="absolute h-[55%] w-[24%] top-[16%] left-1/2 z-10 -translate-x-1/2 flex flex-col gap-y-[2rem] overflow-y-auto max-h-full overscroll-contain"
              ...
              <div className="sticky top-[calc(var(--hd)+1rem)] flex flex-col border-[var(--cl-third)] border-[0.1rem] rounded-[1rem] p-[1.6rem]  gap-y-[2rem]">
                ...
                {isSubscribed && (
                  <Alert className="mb-4">
                    <AlertDescription className="text-sm text-green-700">
                      Real-time seat updates active - You will be notified if seats are selected by
                      others
                    </AlertDescription>
                  </Alert>
                )}
                ...
                {searchBarData.totalPerson > 0 && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800">
                    ...
                    <p
                      className={`mt-2 font-semibold ${selectedSeatsInfo.length === passengersNeedingSeats ? "text-green-700" : selectedSeatsInfo.length > passengersNeedingSeats ? "text-red-700" : "text-orange-700"}`}
                    >
                      Seats selected: {selectedSeatsInfo.length} / {passengersNeedingSeats}
                      {selectedSeatsInfo.length > passengersNeedingSeats &&
                ...
                <div className="flex flex-col gap-y-[0.8rem]">
                  <p className="text-mn text-[var(--cl-red)] font-medium">Note</p>
                  <ul className="flex flex-wrap gap-[1.2rem]">
                    <li className="flex gap-x-[0.4rem] items-center">
                      <span className="w-[2rem] h-[1.6rem] block flex-shrink-0 bg-[var(--cl-pri)] rounded" />
                      <p className="text-[var(--cl-pri)] text-mn">Selected</p>
                    </li>
                    <li className="flex gap-x-[0.4rem] items-center">
                      <span className="w-[2rem] h-[1.6rem] block flex-shrink-0 bg-[var(--cl-four)] rounded" />
                      <p className="text-[var(--cl-four)] text-mn">Selected</p>
                    </li>
                    <li className="flex gap-x-[0.4rem] items-center">
                      <span className="w-[2rem] h-[1.6rem] block flex-shrink-0 bg-[var(--cl-seven)] rounded" />
                      <p className="text-[#57595B] text-mn">Empty</p>
                    </li>
                    <li className="flex gap-x-[0.4rem] items-center">
                      <span className="w-[2rem] h-[1.6rem] block flex-shrink-0 bg-gray-400 rounded opacity-50" />
                  ...
                  <div className="flex gap-x-[2rem]">
                    <ul className="flex flex-col gap-y-[0.4rem] gap-x-[1.2rem] ">
                      ...
                      <li className="text-sm text-[var(--cl-four)] uppercase">ES: Economy Smart</li>
                      <li className="text-sm text-[var(--cl-four)] uppercase">EF: Economy Flex</li>
                    </ul>
                    <ul className="flex flex-col gap-y-[0.4rem] gap-x-[1.2rem] ">
                      <li className="text-sm text-[var(--cl-pri)] uppercase">BS: Business Smart</li>
                      <li className="text-sm text-[var(--cl-pri)] uppercase">BF: Business Flex</li>
                    </ul>
                  </div>
                  ...
                  <div className="flex flex-col gap-y-[1rem]">
                    ...
                    >
                      {isSaving ? tCommon("loading") : tBk("continuePayment")}
                    </Button>
                  </div>
                </div>
              </div>
