...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('[booking-state/cabin] Non-JSON response from backend:', responseText.substring(0, 500));
      return NextResponse.json(
    ...
    console.error('[booking-state/cabin] Error:', error?.message || error);
    return NextResponse.json(
