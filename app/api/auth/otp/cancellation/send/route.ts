...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId || !bookingId) {
      return NextResponse.json({ message: "userId and bookingId are required" }, { status: 400 });
    }
    ...
    // Call backend API
    const response = await fetch(`${BACKEND_API_URL}/api/v1/auth/otp/cancellation/send`, {
      method: "POST",
      headers: {
