...
export async function GET(
  req: NextRequest,
  context: { params: Promise<{ ticketId: string }> | { ticketId?: string } }
) {
  try {
    ...
    const response = await fetch(
      `${BACKEND_API_URL}/api/v1/bookings/tickets/${encodeURIComponent(ticketId)}/info`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data.message || "Failed to get ticket info" },
