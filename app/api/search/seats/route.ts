...
export async function GET(req: NextRequest) {
  try {
    // Lấy access token từ header (optional - backend sẽ tự lấy cabinType từ Redis nếu có)
    const authHeader = req.headers.get('authorization');

    // Lấy query parameters
    const { searchParams } = new URL(req.url);
    const flightInstanceId = searchParams.get('flightInstanceId');
    const cabinType = searchParams.get('cabinType'); // Optional
    ...
    const response = await fetch(
      `${BACKEND_API_URL}/api/v1/search/seats?${queryParams.toString()}`,
      {
        method: 'GET',
        headers,
      }
    ...
    try {
      ...
      console.error('[search/seats] Non-JSON response from backend:', responseText.substring(0, 500));
      return NextResponse.json(
    ...
    console.error('[search/seats] Error:', error?.message || error);
    return NextResponse.json(
