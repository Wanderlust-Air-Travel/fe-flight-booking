...
export function useRealtime() {
  ...
  useEffect(() => {
    ...
    const sessionId =
      typeof window !== "undefined" ? sessionStorage.getItem("guest_session_id") : null;
    ...
    newSocket.on("connect_error", (err) => {
      ...
      if (reconnectAttemptsRef.current >= maxReconnectAttempts) {
        ...
        console.warn(
          `[Realtime] Connection error (attempt ${reconnectAttemptsRef.current}/${maxReconnectAttempts}):`,
          err.message
        );
      }
    });
