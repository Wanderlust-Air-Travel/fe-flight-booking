...
import { SeatAvailabilityChange, SeatAvailabilityUpdateEvent } from '@/types/realtime-types';
...
export function useSeatAvailability(flightInstanceId: string | null) {
	...
	const subscribeToSeatAvailability = useCallback(() => {
		...
		const unsubscribeUpdate = subscribe('seat-availability:update', (data: SeatAvailabilityUpdateEvent) => {
			if (data.flightInstanceId === flightInstanceId) {
