...
const CheckInSeatSelectionContent = () => {
  ...
  const [selectedSeatsByFlight, setSelectedSeatsByFlight] = useState<
    Map<string, Array<{ flightSeatId: string; seatNumber: string }>>
  >(new Map());
  ...
  const findSeatBySeatId = useCallback(
    (seatId: string, seatGroups: SeatGroup[]): SeatItem | null => {
      for (const group of seatGroups) {
        const seat = group.list?.find(
          (seat) =>
      ...
      .then((res) => {
        if (res.status === 200 && res.data) {
          ...
          if (res.data.status !== "paid" && res.data.status !== "confirmed") {
            setError(
              `Đặt chỗ này có trạng thái "${res.data.status}". Chỉ có thể làm thủ tục cho đặt chỗ đã thanh toán hoặc đã xác nhận.`
            );
            setLoading(false);
            return;
          }
  ...
  useEffect(() => {
    ...
    const fetchSeatMaps = async () => {
      const flightInstanceIds = [
        ...new Set(bookingData.segments.map((seg: any) => seg.flightInstanceId)),
      ];
      ...
      for (const flightInstanceId of flightInstanceIds) {
        ...
        const segment = bookingData.segments.find(
          (seg: any) => seg.flightInstanceId === flightInstanceId
        );
  ...
  const handleSeatToggle = useCallback(
    (flightInstanceId: string, _cabinType: "business" | "economy") => {
      return async (seatId: string, checked: boolean) => {
        ...
        if (!seat.flightSeatId) {
          console.error(
            `[handleSeatToggle] Seat ${seat.seatNumber} has no flightSeatId. Cannot proceed.`
          );
        ...
        setSelectedSeatsByFlight((prev) => {
          ...
          const passengersNeedingSeats =
            passengersNeedingSeatsByFlight.get(flightInstanceIdStr) || 0;

          if (checked) {
            // Check if seat already selected (by flightSeatId)
            if (currentSeats.some((s) => s.flightSeatId === seat.flightSeatId)) {
              console.log(
                `[handleSeatToggle] Seat ${seat.seatNumber} (${seat.flightSeatId}) already selected`
              );
              return prev;
            }
            ...
            if (currentSeats.length >= passengersNeedingSeats) {
              setSubmitError(
                `Bạn chỉ có thể chọn ${passengersNeedingSeats} ghế cho ${passengersNeedingSeats} hành khách (trừ trẻ sơ sinh).`
              );
              return prev;
            }
            ...
            console.log(
              `[handleSeatToggle] Selected seat ${seat.seatNumber} (${seat.flightSeatId}) for flight ${flightInstanceIdStr}. Total: ${newSeats.length}`
            );
          ...
          console.log(
            `[handleSeatToggle] Deselected seat ${seat.seatNumber} (${seat.flightSeatId}) for flight ${flightInstanceIdStr}. Remaining: ${filteredSeats.length}`
          );
  ...
  const handleCheckIn = useCallback(async () => {
    console.log("[handleCheckIn] START - Check-in submission initiated", {
      ...
      selectedSeatsByFlightEntries: Array.from(selectedSeatsByFlight.entries()).map(([k, v]) => ({
        flightId: k,
        seatsCount: v.length,
      })),
    });
    ...
    let checkInSegments: Array<{
      flightInstanceId: string;
      seats: Array<{ flightSeatId: string; seatNumber: string }>;
    }> = [];

    try {
      // Prepare segments for check-in
      // CRITICAL: Ensure flightInstanceId is string and seats have correct format
      checkInSegments = Array.from(segmentsByFlight.entries()).map(
        ([flightInstanceId, _segments]) => {
          // CRITICAL: Ensure flightInstanceId is string for Map lookup
          const flightInstanceIdStr = String(flightInstanceId);
          ...
          if (validSeats.length !== seats.length) {
            console.warn(
              `[handleCheckIn] Some seats have invalid format. Valid: ${validSeats.length}, Total: ${seats.length}`
            );
          }

          if (validSeats.length === 0) {
            console.error(
              `[handleCheckIn] No valid seats found for flight ${flightInstanceIdStr}. This should not happen after validation.`
            );
          }
      ...
      if (totalSeats === 0) {
        ...
        setSubmitError(
          "Không có ghế ngồi nào được chọn. Vui lòng chọn ghế trước khi hoàn tất làm thủ tục."
        );
        setIsSubmitting(false);
        return;
      }
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        if (response.data?.alreadyCheckedIn) {
          ...
          console.log(
            "[handleCheckIn] Booking was already checked in, redirecting to confirmation"
          );
          router.push(
            `/check-in/confirmation?bookingCode=${encodeURIComponent(bookingCode)}&ticketCount=${response.data.ticketCount || 0}&alreadyCheckedIn=true`
          );
        } else {
          // New check-in completed successfully
          console.log(
            "[handleCheckIn] Check-in completed successfully, redirecting to confirmation"
          );
          router.push(
            `/check-in/confirmation?bookingCode=${encodeURIComponent(bookingCode)}&ticketCount=${response.data.ticketCount || 0}`
          );
        }
      } else {
      ...
      if (
        errorMessage.includes("already been checked in") ||
        errorMessage.includes("Tickets have already been issued")
      ) {
        ...
        const ticketCount =
          ...
          bookingData?.passengers?.filter((p: any) => p.passengerType !== "INF").length ||
          0;
        console.log(
          "[handleCheckIn] Booking already checked in (from error), redirecting to confirmation"
        );
        router.push(
          `/check-in/confirmation?bookingCode=${encodeURIComponent(bookingCode)}&ticketCount=${ticketCount}&alreadyCheckedIn=true`
        );
        return;
      }
    ...
    passengersNeedingSeatsByFlight,
    router,
  ]);
  ...
  const currentFlightInstanceId = firstFlightInstanceId
    ? String(firstFlightInstanceId)
    : String(bookingData?.segments?.[0]?.flightInstanceId || "");
      ...
      <main className="flex flex-col pt-[var(--hd)] gap-y-[var(--rowY)]">
        ...
        <div className="container">
          <div className="max-w-md mx-auto py-8 md:py-12 lg:py-16">
            ...
            <Button onClick={() => router.push("/check-in")} className="mt-4 w-full">
              Quay lại
            </Button>
          </div>
        </div>
    ...
    <main className="flex flex-col pt-[var(--hd)] gap-y-[var(--rowY)]">
      ...
      <div className="container">
        <div className="bg-white rounded-lg shadow-md p-5 sm:p-6 mb-6">
          <h2 className="text-xl sm:text-2xl font-bold mb-4 text-[var(--cl-pri)]">
            Thông tin đặt chỗ
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 text-base sm:text-lg">
            <div className="flex justify-between sm:block">
              ...
              <span className="ml-2 sm:ml-0 font-mono font-bold text-[var(--cl-pri)]">
                {bookingData.pnrCode}
              </span>
            </div>
            <div className="flex justify-between sm:block">
              <span className="text-gray-500 font-medium">Trạng thái:</span>
              <span className="ml-2 sm:ml-0 capitalize font-semibold text-[var(--cl-five)]">
                {bookingData.status}
              </span>
            </div>
            <div className="flex justify-between sm:block">
              <span className="text-gray-500 font-medium">Số hành khách:</span>
              <span className="ml-2 sm:ml-0 font-semibold">
                ...
                <div className="absolute h-[55%] w-[24%] top-[16%] left-1/2 z-10 -translate-x-1/2 flex flex-col gap-y-[2rem] overflow-y-auto max-h-full">
                  ...
                  {seatBusiness &&
                    (() => {
                      ...
                      const flightInstanceIdStr =
                        currentDisplayFlightId ||
                        String(Array.from(segmentsByFlight.keys())[0] || "");
                      const currentCabinType =
                        bookingData?.segments?.find(
                          (s: any) => String(s.flightInstanceId) === flightInstanceIdStr
                        )?.cabinType || "economy";
                      const currentSelectedSeats =
                        selectedSeatsByFlight.get(flightInstanceIdStr) || [];
                  ...
                  {Array.from(segmentsByFlight.entries()).length === 1 ? (
                    ...
                    <>
                      ...
                      <p className="text-sm text-gray-500 font-medium">
                        Cần chọn
                        {Array.from(passengersNeedingSeatsByFlight.values()).reduce(
                          (a, b) => a + b,
                          0
                        )}
                        ghế cho
                        {bookingData.passengers?.filter((p: any) => p.passengerType !== "INF")
                          .length || 0}
                        hành khách
                      </p>
                    </>
                      ...
                      return (
                        <div key={flightStr} className="border-t pt-3">
                          <p className="text-sm font-semibold text-gray-700">
                            {segments[0]?.flightNumber || `Chuyến ${flightStr.substring(0, 8)}`}
                          </p>
                  ...
                  disabled={
                    ...
                    (() => {
                      ...
                      for (const [
                        flightId,
                        passengersNeeded,
                      ] of passengersNeedingSeatsByFlight.entries()) {
...
export default CheckInSeatSelectionContent;
