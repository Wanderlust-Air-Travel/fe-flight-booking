...
import type { BaggageAllowance } from "@/types/admin/baggage-allowance-type";
import type { FareClass } from "@/types/admin/route-fare-price-type";
import {
  CheckCircle2,
  ChevronLeft,
  ChevronRight,
...
export default function BaggageAllowancesPage() {
  ...
  const handleCreate = async () => {
    try {
      await axiosInstance.post("/api/admin/baggage-allowances", {
        ...
        checkedBaggageKg: formData.checkedBaggageKg
          ? Number.parseFloat(formData.checkedBaggageKg)
          : null,
        checkedBaggagePieces: formData.checkedBaggagePieces
          ? Number.parseInt(formData.checkedBaggagePieces)
          : null,
        carryOnKg: Number.parseFloat(formData.carryOnKg),
        carryOnPieces: Number.parseInt(formData.carryOnPieces),
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axiosInstance.put(
        `/api/admin/baggage-allowances/${editingAllowance.baggageAllowanceId}`,
        {
          checkedBaggageKg: formData.checkedBaggageKg
            ? Number.parseFloat(formData.checkedBaggageKg)
            : null,
  ...
  const filteredAllowances = baggageAllowances.filter((allowance) => {
    ...
    const fareClassText = fareClass
      ? `${fareClass.fareClassCode} ${fareClass.description || ""} ${fareClass.name || ""}`.toLowerCase()
      : "";
    ...
    <div className="space-y-6">
                ...
                <div className="grid gap-6 overflow-y-auto max-h-[calc(90vh-200px)] pr-2">
                  <div className="grid gap-3">
                    ...
                    >
                      ...
                      <SelectContent className="text-base">
                        {fareClasses.map((fc) => (
                          <SelectItem
                            key={fc.fareClassCode}
                            value={fc.fareClassCode}
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="grid gap-3">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, checkedBaggageKg: e.target.value })
                        }
                        placeholder="20"
                        min="0"
                    ...
                    <div className="grid gap-3">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, checkedBaggagePieces: e.target.value })
                        }
                        placeholder="1"
                        min="0"
                        className="h-14 text-base"
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="grid gap-3">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, carryOnPieces: e.target.value })
                        }
                        placeholder="1"
                        min="0"
                        className="h-14 text-base"
                  ...
                  <div className="grid gap-3">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, carryOnDimensions: e.target.value })
                      }
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="grid gap-3">
                      <Label className="text-base font-semibold text-gray-700">Trạng thái</Label>
                      <div className="flex items-center h-14 px-4 border rounded-md bg-white hover:bg-gray-50 transition-colors">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({ ...formData, isDomestic: e.target.checked })
                          }
                          className="h-5 w-5 rounded border-gray-300 text-[#00558f] focus:ring-2 focus:ring-[#00558f]/20 cursor-pointer"
                        />
                        <Label
                          htmlFor="isDomestic"
                          className="text-base font-semibold text-gray-700 cursor-pointer ml-3 flex-1"
                        >
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-2xl font-bold text-gray-900">
                Danh sách quy định hành lý
              </CardTitle>
              <CardDescription className="mt-2 text-base font-medium text-gray-600">
                Hiển thị {filteredAllowances.length} / {totalItems.toLocaleString("vi-VN")} quy định
                hành lý
              </CardDescription>
            </div>
          </div>
      ...
      >
        <DialogContent className="sm:max-w-[100vw] md:max-w-[90vw] lg:max-w-[80vw] xl:max-w-[70vw] 2xl:max-w-[60vw] 3xl:max-w-[70vw] max-w-5xl max-h-[90vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle className="text-3xl font-bold text-gray-900">
              Chỉnh sửa quy định hành lý
            </DialogTitle>
            <DialogDescription className="text-base text-gray-600 mt-2">
              Cập nhật thông tin quy định hành lý cho
              {editingAllowance?.fareClassCode}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-6 overflow-y-auto max-h-[calc(90vh-200px)] pr-2">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="grid gap-3">
                <Label
                  htmlFor="edit-checkedBaggagePieces"
                  className="text-base font-semibold text-gray-700"
                >
                  Số lượng hành lý ký gửi
                  <span className="text-gray-400 font-normal text-sm ml-2">(tùy chọn)</span>
            ...
            <div className="grid gap-3">
              <Label
                htmlFor="edit-carryOnDimensions"
                className="text-base font-semibold text-gray-700"
              >
                Kích thước hành lý xách tay (cm)
                <span className="text-gray-400 font-normal text-sm ml-2">(tùy chọn)</span>
