...
import type { AirportItem } from "@/types/airport-item";
...
const FlightSearchBar = () => {
  ...
  useEffect(() => {
    const fetchAirports = async () => {
      try {
        ...
        setAirportsError(
          error.response?.data?.message || error.message || "Failed to load airports"
        );
  ...
  const isFormValid = (): boolean => {
    ...
    if (data.minor < 0 || Number.isNaN(data.minor)) {
      return false;
    }

    // Ensure adult is a valid number
    if (Number.isNaN(data.adult)) {
      return false;
    }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const hasOutboundFlights =
        response.data?.outbound &&
        Array.isArray(response.data.outbound) &&
        response.data.outbound.length > 0;
      const hasInboundFlights =
        data.service === "round_trip"
          ? response.data?.inbound &&
            Array.isArray(response.data.inbound) &&
            response.data.inbound.length > 0
    ...
    <>
      <div className="flex flex-col md:flex-row w-full items-stretch bg-white border-[#CBD4E6] border-[0.1rem]">
        <div className="w-full md:w-[20%] border-b md:border-b-0 md:border-r-[0.1rem] border-[#cbd4e6] relative">
          ...
          />
          {airportsError && (
            <p className="text-sm text-red-500  px-4 absolute top-[105%] line-clamp-1 left-0 w-full">
              Failed to load airports. Please refresh the page.
            </p>
          )}
        </div>
        <div className="w-full md:w-[20%] border-b md:border-b-0 md:border-r-[0.1rem] border-[#cbd4e6] relative">
          ...
          />
          {airportsError && (
            <p className="text-sm text-red-500  px-4 absolute top-[105%] left-0 w-full line-clamp-1">
              Failed to load airports. Please refresh the page.
            </p>
          )}
        </div>
