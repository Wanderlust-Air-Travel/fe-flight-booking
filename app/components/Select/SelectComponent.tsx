...
import type { SelectComponentProp } from "@/types/select-component-type";
...
export function SelectComponent({
  ...
  return (
    <Select value={value} onValueChange={onChange} disabled={disabled}>
      <SelectTrigger
        className="w-full  border-none outline-none text-start !h-[4.8rem] shadow-none text-sm sm:text-base md:text-[1.4rem] !text-[var(--cl-pri)] px-2 sm:px-3 md:px-[1rem] select-none"
        disabled={disabled}
      >
        <div className="flex gap-x-2 sm:gap-x-[0.8rem] w-full items-center custom-sl">
          ...
          />
          <SelectValue
            className="!w-full text-sm sm:text-base md:text-[1.4rem]"
