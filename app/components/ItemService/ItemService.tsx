...
const ItemService = ({ image, title, link, startDate, endDate, service, price }: ItemServiceProp) => {
    ...
    const buildSearchHref = () => {
        try {
            ...
            return `/search/flights?origin=${originCode}` +
        ...
        <div className="group flex flex-col shadow2 rounded-[1rem] overflow-hidden h-full hover:shadow3 transition-all duration-300 bg-white">
            ...
            <div className="p-[1rem] md:p-[1.2rem] flex flex-col gap-y-[0.8rem]">
                ...
                <div className="flex gap-x-[0.8rem] justify-between items-end">
                    ...
                    <Link href={searchHref} className="peer arrowLink w-[3.6rem] h-[3.6rem] md:w-[4rem] md:h-[4rem] bg-[var(--cl-third)] rounded-md flex justify-center items-center flex-shrink-0 hover:bg-[var(--cl-four)] transition ease-linear">
